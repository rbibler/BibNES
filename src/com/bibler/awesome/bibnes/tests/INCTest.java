...
public class INCTest extends InstructionTest {
	...
	private final int ZERO_PAGE = 0xE6;
	private final int ZERO_PAGE_X = 0xF6;
	private final int ABSOLUTE = 0xEE;
	private final int ABSOLUTE_X = 0xFE;
	...
	public void testZeroPage() {
		...
		writeRom(0x23, 0x55);
		runNCycles(5);
		assertEquals(0x56, readRom(0x23));
		assertFalse(zeroFlag());
		assertFalse(signFlag());
	}
	
	public void testZeroPageWithOverflowflow() {
		initializeCPU();
		resetCPU();
		writeRom(0x40, ZERO_PAGE);
		writeRom(0x41, 0x23);
	...
	public void testZeroPageX() {
		...
		setXIndex(5);
		runNCycles(6);
		assertEquals(0x56, readRom(0x49));
		assertFalse(zeroFlag());
		assertFalse(signFlag());
	}
	...
	public void testAbsolute() {
		...
		runNCycles(6);
		assertEquals(0x56, readRom(0x4400));
		assertFalse(zeroFlag());
		assertFalse(signFlag());
	}
	...
	public void testAbsoluteX() {
		...
		setXIndex(5);
		runNCycles(7);
		assertEquals(0x56, readRom(0x4405));
		assertFalse(zeroFlag());
		assertFalse(signFlag());
	}
